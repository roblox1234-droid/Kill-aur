...
local function getSilentTarget()
    ...
    -- Highlight.Enemy.HighlightHolder
    ...
    if world then
        for _, w in ipairs(world:GetChildren()) do
            local ent = w:FindFirstChild("Entities")
            if ent then
        ...
        if Config.SilentAim and method == "Raycast" and self == workspace then
            ...
            if typeof(origin) == "Vector3" and typeof(direction) == "Vector3" then
                ...
                if target and math.random(100) <= Config.SilentAimChance then
                    ...
                    lastSilentTarget = target.Parent.Name
                    return oldNamecall(self, origin, newDir, select(3, ...))
                end
            end
        end

        if Config.SilentAim and (method == "FindPartOnRayWithIgnoreList"
            or method == "FindPartOnRay" or method == "FindPartOnRayWithWhitelist") then
            local args = {...}
            local ray = args[1]
...
local function getESPColor(player, distance)
    ...
    if Config.TeamCheck then
        ...
        if lpTeam and player.Team == lpTeam then isSameTeam = true end
    end
    if isSameTeam then
...
local function getAuraTargets()
        ...
        if model:IsA("Model") and model ~= LP.Character then
            ...
            if pRoot and pHum and pHum.Health > 0 then
                ...
                if dist <= Config.KillAuraRange then
                    table.insert(targets, {char = model, root = pRoot, dist = dist})
                end
            end
        end
    end
    ...
    local plrFolder = workspace:FindFirstChild("Players")
    if plrFolder then
        for _, m in ipairs(plrFolder:GetChildren()) do checkModel(m) end
    end
...
-- COMBAT TAB - SILENT AIM ВВЕРХУ
makeToggle(pages.Combat, "SILENT AIM (T)", "SilentAim")
makeToggle(pages.Combat, "Silent Ignore Walls", "SilentAimVisible")
makeSlider(pages.Combat, "Silent FOV", 100, 2000, Config.SilentAimFOV, function(v) Config.SilentAimFOV = v end)
makeSlider(pages.Combat, "Silent Chance %", 10, 100, Config.SilentAimChance, function(v) Config.SilentAimChance = v end)

-- Silent Aim Bone button
local silentBones = {"Head", "UpperTorso", "HumanoidRootPart"}
local silentBoneBtn = Instance.new("TextButton")
silentBoneBtn.Size = UDim2.new(1, 0, 0, 30)
silentBoneBtn.BackgroundColor3 = THEME.bg3
silentBoneBtn.BorderSizePixel = 0
silentBoneBtn.Text = "  Silent Bone: " .. Config.SilentAimBone
silentBoneBtn.TextColor3 = THEME.accent
silentBoneBtn.Font = Enum.Font.Gotham
silentBoneBtn.TextSize = 13
silentBoneBtn.TextXAlignment = Enum.TextXAlignment.Left
silentBoneBtn.ZIndex = 1003
silentBoneBtn.Parent = pages.Combat
Instance.new("UICorner", silentBoneBtn).CornerRadius = UDim.new(0, 6)

silentBoneBtn.MouseButton1Click:Connect(function()
    local idx = 1
    for i, m in ipairs(silentBones) do
        if m == Config.SilentAimBone then idx = i break end
    end
    idx = idx + 1
    if idx > #silentBones then idx = 1 end
    Config.SilentAimBone = silentBones[idx]
    silentBoneBtn.Text = "  Silent Bone: " .. Config.SilentAimBone
    showNotify("Bone: " .. Config.SilentAimBone, THEME.accent)
end)
...
UIS.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == SILENT_TOGGLE_KEY then
        Config.SilentAim = not Config.SilentAim
        showNotify("SILENT AIM: " .. (Config.SilentAim and "ON" or "OFF"),
            Config.SilentAim and THEME.green or THEME.danger)
        return
    end
...
showNotify(SCRIPT_NAME .. " загружен! E=ESP T=Silent", THEME.accent)
